...
class Root:
	...
	def open_load_data_window(self):
		...
		self.frame_container = tk.Frame(bg=self.controllers.theme_controller.get_theme("backgroundColor"))
		...
		# ouvre les anciennes données
		self.load_previous_data_button = self.models.buttons.valid_button.create_widget(self.frame_container, self.controllers.language_controller.get_text("previousData"))
		self.load_previous_data_button.configure(command=self.load_previous_data)
		self.load_previous_data_button.pack(pady=25)
